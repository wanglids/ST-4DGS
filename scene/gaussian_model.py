...
class GaussianModel:
    ...
    def restore(self, model_args, checkpoint_iterations):
        ...
        # self._deformation = torch.load(os.path.join(model_args,f'point_cloud/iteration_{checkpoint_iterations}/deformation.pth'))
        # self._deformation_accum = torch.load(os.path.join(model_args, f'point_cloud/iteration_{checkpoint_iterations}/deformation_accum.pth'))
        # self._deformation_table = torch.load(os.path.join(model_args, f'point_cloud/iteration_{checkpoint_iterations}/deformation_table.pth'))
        self.load_model(os.path.join(model_args,f'point_cloud/iteration_{checkpoint_iterations}/'))
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        self._deformation_table = torch.gt(torch.ones((self.get_xyz.shape[0]),device="cuda"),0) # N*1 bool True
    ...
    def add_grid_point(self,new_point,near_point_mask):
        
        new_point1 = copy.deepcopy(new_point)
        new_xyz = nn.Parameter(torch.tensor(new_point1, dtype=torch.float, device="cuda").requires_grad_(True))
        
        new_features_dc = self._features_dc[near_point_mask]
        new_features_rest = self._features_rest[near_point_mask]
        new_opacities = self._opacity[near_point_mask]
        new_scaling = self._scaling[near_point_mask]
        new_rotation = self._rotation[near_point_mask]
        new_deformation_table = self._deformation_table[near_point_mask]
    ...
    def motion_splitting(self,near_point_mask):
        ...
        new_deformation_table = self._deformation_table[selected_pts_mask].repeat_interleave(repeat_num+1, dim=0)
    ...
    def geometry_prune_point(self,neighbors,std):
        ...
        cl, ind = pcd_vector.remove_statistical_outlier(neighbors, std)
